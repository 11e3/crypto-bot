#!/usr/bin/env python3
"""Binance Bidirectional VBO Strategy

양방향 VBO (롱/숏):
- 강세장 (BTC > MA20): VBO 롱 (상방 돌파)
- 약세장 (BTC < MA20): VBO 숏 (하방 돌파)

바이낸스 선물 거래 전용 (양방향 포지션 가능)

Usage:
    python backtest_binance_bidirectional_vbo.py
    python backtest_binance_bidirectional_vbo.py --start 2022-01-01 --end 2024-12-31
"""
...
# Backtest
# =============================================================================
def backtest_bidirectional_vbo(symbol: str, start: str | None = None, end: str | None = None) -> tuple[dict, pd.DataFrame]:
    """Backtest Bidirectional VBO strategy."""
    df = load_data(symbol)
    ...
    for date, row in df.iterrows():
        ...
        if position != 0:
            ...
            if position_type == 'LONG':
                # Long exit: prev_close < prev_ma5 OR prev_btc < prev_btc_ma
                ...
                # Short cover: prev_close > prev_ma5 OR prev_btc > prev_btc_ma
                if row['prev_close'] > row['prev_ma5']:
                    sell_signal = True
                    sell_reason = "Short Cover (MA5)"
                elif row['prev_btc_close'] > row['prev_btc_ma']:
                    sell_signal = True
        ...
        if position == 0:
            ...
            if buy_signal:
                ...
                if position_type == 'LONG':
                    # Long position: buy coins with all cash
                    position = (entry_value - entry_fee) / entry_price
                    position_entry_price = entry_price
                    cash = 0.0
                else:  # SHORT
                    # Short position: keep cash, track notional short position
                    position = -(entry_value - entry_fee) / entry_price
                    position_entry_price = entry_price
                    # Don't zero out cash for short! Keep it as collateral

        # Calculate equity (mark-to-market)
        if position > 0:  # Long
            equity = cash + position * row['close']
        elif position < 0:  # Short
            # Short equity = initial capital + PnL
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description='Backtest Binance Bidirectional VBO')
    ...
    print("=" * 100)
    print("BINANCE BIDIRECTIONAL VBO STRATEGY")
    print("=" * 100)
    print(f"\nBull Market (BTC > MA{BTC_MA}): VBO LONG (상방 돌파)")
    print(f"Bear Market (BTC < MA{BTC_MA}): VBO SHORT (하방 돌파)")
    ...
    for symbol in symbols:
        ...
        try:
            metrics, _ = backtest_bidirectional_vbo(symbol, args.start, args.end)
